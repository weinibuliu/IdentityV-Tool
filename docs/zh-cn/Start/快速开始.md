...
## 下载
IVT 的版本号结构为 IdentityV-Tool-(system/操作系统)-(arch/架构)-(version/版本号)。

### Windows (Windows8.1及以上)
- 对于 x86_64 架构：请下载 `IdentityV-Tool-x86_64-(version).zip` 。（大多数搭载 Windows的电脑均为 x86_64 架构）
- 对于 ARM 架构：请下载 `IdentityV-Tool-aarch64-(version).zip` 。
